  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      document.querySelectorAll("[data-parallax]").forEach((el) => {
        if (mobile) { el.style.transform = ""; return; }
...
function Nav({ theme, onToggleTheme }) {
  ...
  return (
    <header className={"nav" + (stuck ? " stuck" : "")}>
      ...
      {open && (
        <div className="wrap-wide" style={{ paddingBottom: "0", paddingTop: "0", display: "flex", flexDirection: "column", gap: "0" }}>
          {NAV.map((n) => (
            <a key={n.href} href={n.href} className="nav-link nav-link-mobile" data-hot onClick={() => setOpen(false)}>{n.label}</a>
          ))}
        </div>
      )}
    </header>
